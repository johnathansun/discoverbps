  ...
  def index
    ...
    if student_schools && student_schools[:choiceList] && student_schools[:studentInfo]
      ...
      @student = Student.where(session_id: session[:session_id], 
        ...
        first_name: student_info[:FirstName].try(:strip), 
        last_name: student_info[:LastName].try(:strip), 
        grade_level: student_info[:Grade].try(:strip).try(:gsub, /^0/, ''),
        street_number: student_info[:Streetno].try(:strip),
        street_name: student_info[:Street].try(:strip),
        neighborhood: student_info[:City].try(:strip),
        zipcode: student_info[:ZipCode].try(:strip),
