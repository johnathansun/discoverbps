class PreferenceCategory < ActiveRecord::Base
  ...
  scope :include_in_preferences_panel, -> { where(include_in_preferences_panel: true) }
	...
	def self.active(grade)
		...
		PreferenceCategory.eager_load(:preferences).where("preference_categories.include_in_preferences_panel = ? AND preferences.grade_#{grade_level} = ?", true, true).order("preference_categories.sort_order")
	end
